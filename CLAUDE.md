...
2. **Logic must be node-safe.** Game logic modules must not touch `document`/`window`/canvas/AudioContext at import time or in logic paths — that's what makes them unit-testable. Only `main.ts`, `devtuner.ts` (dev-only, flag-gated, never imported by anything but `main.ts`), `engine/renderer.ts`, `engine/audio.ts`, `engine/levelio.ts` (guarded fetch/storage/clipboard), `engine/link.ts` (guarded `window.open`) and `Input.attach()` may touch browser APIs, and even those must be importable in node (guard access, don't execute it at import). **`Input.attach` owns the pointer**, and converts it to view space via `screenToView` on the way in — which is what lets `EditorScene` unit-test with the mouse included.
