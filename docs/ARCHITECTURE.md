...
Everything draws into an offscreen **960×540** canvas, then `present()` blits it to the visible canvas at the largest scale that fits, centred with letterbox bars. The scale is **snapped to quarter steps, not to whole multiples**: flooring it made the frame's size a step function of the window, and the step landed in the worst possible place — a browser windowed on a 1080p display fits 1.74× and floored to 1, while the same window on a 1440p display fits 2.41× and floored to 2, so one display drew the game at half the linear size of the other for no reason a player could see. The art is antialiased paths rather than a pixel grid, so it survives a resampled blit; it does not survive being half the size. Filling the window exactly overcorrects, though — an arbitrary scale is an arbitrary resample, and the frame goes soft in proportion to how odd the ratio is — so a magnifying scale floors to the quarter below it. That keeps every common window within about 14% of a perfect fit (no cliff: the shortfall is bounded by `0.25 / raw` and shrinks as displays grow) while landing on a rational magnification, and on an exact whole one whenever the window is near a multiple of the view. Below 1× nothing snaps: a window smaller than the view is resampling everything it draws already. Offsets stay integral (rounded, not floored — the tight axis lands a few ulps either side of zero and flooring turns that into an off-by-one shift).
...
| `present` | n/a — `imageSmoothingEnabled = !Number.isInteger(scale)` | At a whole-number scale (fullscreen 1080p is exactly 2×) every source pixel lands on a whole block of destination pixels, and smoothing could only soften a frame that is already exact. At a fractional scale it is the difference between a resampled frame and one with ragged, unevenly-doubled rows. The offscreen buffer sets the flag **on** unconditionally, for a different reason: the aberration pass blits it at sub-pixel offsets. |
