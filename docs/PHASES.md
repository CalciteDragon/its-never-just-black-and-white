...
And then sharing, which is the amendment that deleted a phase-7 decision outright. **Decision 7's transport was aimed at the wrong destination.** Behaviour-detection over `import.meta.env.DEV` was right and stands; `POST /__level` was not, because a dev-server middleware writing `src/levels/<id>.json` makes the editor's one way out work for exactly one person — whoever has this repo checked out. So `saveLevel` became **`exportLevel`**: a `<id>.json` download, identical in dev, in `vite preview` and on a static host, still byte-identical to `serializeLevel` so the file can be committed verbatim, with the clipboard and then `bw.editor.draft` as reported fallbacks. `levelSink` was **removed** rather than left behind — a middleware nothing calls is dead code that still looks like a supported route. The rename is not cosmetic: the editor already saves, on every stroke, to the shelf, so calling `Ctrl+S` "save" answered a question nobody was asking and left two real ones open. `Ctrl+S` is now also suppressed *by name* at the window rather than by accident of `KeyS` being a WASD key.

**The pad-into-solid warning is gone.** Phase 5's *As built* nominated the editor as the place to warn about a pad whose facing points into its own geometry, and phase 7's decision 6 built it. Amendment 2 above took the trap away underneath it — a pad that debounces itself no longer re-fires every step — and what was left was a warning fired by a shape authors place on purpose, which is the one thing a warnings panel must never do. `gridWarnings` now covers markers only; `PAD_FACINGS` went with it.

The other direction is new. **`FileDropbox` is a queue, not a callback** — the DOM `drop` (and the OS file picker behind the `+ IMPORT A LEVEL` rows) fills it, and scenes drain it in `update` — because a callback fired from a DOM handler would run scene logic outside the fixed-step loop and put a browser API on a scene's path, which hard rule 2 forbids. `editor/transfer.ts` is the pure half, validating with **the same `validateLevel`** the editor's panel uses and **renaming rather than overwriting**, since the id is the shelf's key.

And **CUSTOM LEVELS**, which is the draft shelf seen from the play side: one store, so a level cannot be playable and un-editable. It is pinned at row 0 of the level select and bound to `C`, because the built-in list grows with every level shipped and a door that sinks below the fold stops being found. A custom run is a **third `PlayContext`** beside `campaign` and `playtest`, and it sits between them exactly: it writes a best time, and it never touches `bw.progress`. GAME-DESIGN §3, §4, §10 and §12 amended.
