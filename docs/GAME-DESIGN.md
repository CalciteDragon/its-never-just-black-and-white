...
Internal resolution **960×540** (`VIEW_W`×`VIEW_H`), scaled to fit the window with letterboxing — in quarter steps, not whole ones, so the frame is close to the same fraction of the window on every display (snapping to integers made a windowed 1080p browser draw the game at half the size a 1440p one did) while still magnifying by a ratio clean enough to stay crisp. Unlike the old build, shapes are drawn **antialiased** — a square that rests at 37° has to have a clean edge. Tiles are **32×32 px** (`TILE`), giving a 30×16.9 tile viewport.
...
The entire game uses two colours plus two sanctioned exceptions.
...
> *(Amended after phase 7 — this originally listed a third token, `accent`: one saturated colour, cool in phase A and warm in phase B, used by particles and nothing else.)* Particles now draw as an **inversion of the frame beneath them** — a white operand under `globalCompositeOperation = 'difference'`, i.e. `255 − backdrop` per channel. A spark over `paper` reads as `ink`, a spark over `ink` reads as `paper`, and a spark crossing an edge does both at once, which the accent could never do. Everything the accent was for survives: the flip is still legible from a single spark — more so, because an inverting spark cannot lag a flip at all — and the palette is now literally two colours rather than two plus a licence. `accent` and `accentRgba` are **gone** from `Palette`; particles were their only caller in `src/`, and an API kept alive by its own tests is the placeholder the phase 3/4 rule forbids.
...
| Effect | Response |
| --- | --- |
| Vignette | alpha lerps `VIGNETTE_MIN → VIGNETTE_MAX`; an `ink` tint fades in over the top half of the range, peaking at `VIGNETTE_TINT_MAX`. Ink, and nothing more saturated (§2 — the game has no third colour), so speed deepens the frame rather than staining it |
...
**The credits roll on the sheet the ending leaves behind** *(added after 0.2)*. `finish` sends the finale to `CreditsScene` rather than straight to the results, and that scene draws `drawFinaleSweep` — the veil's own conical sweep, factored out — at full alpha on the clock `PlayScene` hands it. The first frame of the credits is therefore the last frame of the ending, pixel for pixel: the same swirl, still turning at the same angle, with nothing added yet. The `paper` scrim that buys the words their contrast eases in over 1.2 s, and the post pass is skipped outright, because `applyPost(0)` is still a `VIGNETTE_MIN` darkening and there is no way to ask for less of it — dropping a vignette onto the frame the ending had just finished clearing would be the one cut the whole six seconds were built to avoid.

The words SCROLL, for that same reason: nothing has been edited since the goal, and a screen of cards would put the first cut here. They come up in stanzas — the title at scale 5, the line that answers it and the names at 3, the tools at 2, the sign-off at 4 — each line fading in over the bottom edge and out over the top rather than being cut in half mid-glyph. The roll runs all the way OUT: every line arrives and leaves the same way, which is what makes it a roll rather than a slideshow with a last slide, and what `THANKS FOR PLAYING` leaves behind is the screen the ending made, with nothing on it again. After 1.2 s of that, the whole frame fades to `paper` over two seconds and hands the run to the results screen with its time intact — `paper` because that is what the results screen is drawn on, so the seam between the two lands where there is nothing on either of them to see it. The credits are inserted before the results, not instead of them. One press starts the fade and the next ends the screen, so a skip still leaves rather than cutting.

There is **no drop shadow** under the text; the scrim carries the contrast alone, which is what sets it at 0.45 — at that depth the worst hue on screen (the yellow-green arc) reads at 3.5:1 against `ink`, clearing the large-text threshold, and the roll is nothing but large text. The layout is a pure function (`creditsLayout`, `creditsRoll`, `creditsScrim`, `creditsFadeOut`, `creditLineAlpha`) and unit-tested, like the ending's curve before it.

What is still unbuilt is the level it ends: `black-and-white.json` exists and is what the ending and the credits are keyed to, but it is not yet in `LEVELS`.
...
- **`engine/palette.ts`** *(new)* — `class Palette { phase: 0 | 1; flip(): void; paper: string; ink: string }`, plus `paperRgba(a)` and `inkRgba(a, phase?)`. Pure. The single source of every colour in the game. *(Phase 5 adds `inkRgba`, whose optional `phase` exists so the death fade can be sampled once and held: the palette resets to phase A at the fade's peak, and under a live token the screen would jump from white to black at exactly the covered moment. Amended after phase 7: `accent` and `accentRgba` are gone with the accent itself — see §2 — and the module now also exports `INVERT_MASK`, the particles' white `difference` operand, alongside the aberration's channel masks. Those are compositing operands, not palette colours.)*
- **`engine/renderer.ts`** — 960×540 offscreen buffer, antialiased, scaled-to-fit present. Adds `applyPost(speedNorm)` for vignette + chromatic aberration, and world/UI space draws. *(Phase 7 adds `screenToView(canvasW, canvasH, clientX, clientY)`, the exact inverse of `present`'s blit, returning an `inFrame` flag rather than clamping. It is the one place in the project where the letterbox arithmetic lives, and therefore the one place it can be wrong — dropping the offset is a fifteen-tile error at a 3840×1080 window, not a sub-pixel one.)*
- **`engine/particles.ts`** — pooled system, drawn as an inversion of the frame: `class ParticleSystem { aliveCount; emit(x, y, vx, vy, life, size, gravity, drag): boolean; update(dt); render(ctx, camX, camY); clear() }` plus the emitters `spawnDust`, `spawnBurst`, `spawnSplash`, `spawnRing`, `spawnStream`. *(Amended in phase 6, three ways. Particles **store no colour**: `render` assigns one `fillStyle` for the whole pass, so sparks invert in flight — a spawn-time colour would leave a flip trailing a cloud of the outgoing phase, and worse, the flip's own ring is emitted before `PlayScene` moves the palette, so the one thing announcing the flip would be the last thing still wearing the old colour. That one `fillStyle` was the live accent in phase 6 and is now `INVERT_MASK` under `difference` — see §2 — with the pass bracketing itself: `difference` on, N `fillRect`s, `source-over` restored, so the HUD, the post pass and the death fade that follow it composite normally. The pool's free-slot scan starts at a **rotating cursor**, making a spawn amortised O(1) instead of O(512), with drop-newest overflow. And the options-bag `spawn`/`ParticleOpts` are **gone**: once the emitter set landed they had no caller in `src/`, and an API kept alive only by its own tests is the placeholder the phase 3/4 rule forbids.)*
